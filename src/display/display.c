...
void update_lower_strip(){
    ...
    switch(Current_Mode){
        case Insert_Mode :
            mvprintw(Screen_Height - 1 , 1 , "Insert");
            break;

        case Normal_Mode :
            mvprintw(Screen_Height - 1 , 1 , "Normal");
            break;
    }
    attroff(A_UNDERLINE);
    ...
    if(log_of_col_pos + log_of_line_pos + 17/*the extra 6 are for the mode*/ < Screen_Width){
        mvprintw(Screen_Height - 1 , Screen_Width - (log_of_col_pos + log_of_line_pos + 11) , "Ln %i,Col %i" , line_pos + 1 , col_pos + 1);
    }
