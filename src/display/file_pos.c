...
void move_ncols_right(size_t cols){
    ...
    if(Col_Pos + cols > simple_file_get_line_len(Current_File , Line_Pos)){
        if(cols > 1){
            Col_Pos = simple_file_get_line_len(Current_File , Line_Pos);
        }
...
void move_nlines_down(size_t lines){
    if(lines == 0) return ;
    if(Line_Pos + lines >= simple_file_get_line_no(Current_File)){
        if(lines > 1){
            Line_Pos = simple_file_get_line_no(Current_File) - 1;
        }
